...
def plot_results(start_time, end_time, soc, battery_power, df):
    # PLOTTING
    ...
    if max_grid_import > peak_threshold:
        ...
        print(
            "max grid import date and time: ", peak_time
        )
        print("#########################")
        ...
        ax1.text(
            x=df_cut.index[int(len(df_cut.index) * 0.5)],  # Place it at ~80% of the x-axis
    ...
    ax1.plot(compression_power.index, -compression_power, label="Compression Power", color="purple")
    ax1.plot(expansion_power.index, expansion_power, label="Expansion Power", color="green")
...
def validate_caes_model(df, results):
    ...
    try:
        ...
        # total_energy_caes = df["grid_import_caes"].sum() + df["pv_self_use_caes"].sum()
        total_energy_caes = (
            results[(el_source, b_el)]["sequences"]["flow"]
            + results[(el_peak_source, b_el)]["sequences"]["flow"]
        ).sum() + results[(pv_link, b_el)]["sequences"]["flow"].sum()
        assert (
        ...
        pv_link_flow = results[(pv_link, b_el)]["sequences"]["flow"]
        assert (pv_link_flow.min() > -0.01), "⚠️ PV link flow should always be positive"
        ...
        assert (pv_production - excess_sink_series).min() >= 0, "⚠️ Only PV should feed the excess sink"

        # 5️⃣ Stored compressed air should match expanded air
        b_air_in_series = results[(compression_converter, b_air_in)]["sequences"]["flow"]
        b_air_out_series = results[(b_air_out, expansion_converter)]["sequences"]["flow"]
        assert (b_air_in_series.sum() - b_air_out_series.sum() < 1e-3), "⚠️ Stored air does not match expanded air"
...
plot_results(start_time, end_time, soc, battery_power, df)
